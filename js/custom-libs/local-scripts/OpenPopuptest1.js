(function () {
    ...
    var MODULE_NAME = "OpenPopuptest1",
...
const im = shmi.requires("visuals.session.ItemManager");    //verwijzing naar de ItemManager
        const controlName = ".my-Dialogbox";                            //verwijzing naar de naam van de popup
        ...
        OnPLCTagChange = im.subscribe([PLC_Tag], (name, value) => { //subscribe in the ItemManager de variabele "PLC_Tag"= OpenPopupBOOL
            ...
            if (value === 1) {                                      //waarde gelijk aan 1
                let dialogbox = shmi.ctrl(controlName);                 //maak variabele popup aan die gekoppeld is aan de ctrl met de popupnaam
                dialogbox.show();                                       //open de popup
            }
            else {
             //   let popup = shmi.ctrl(controlName);                 //wordt de variabele anders 1 dan sluit de popup weer. 
             //   popup.close();
            }
